...
class ConnectionManager {
  ...
  bool _isTrackedConnection(Connection connection) {
    return _connectionPool[connection.serverConfig.hostUrl]?.contains(
          connection,
        ) ??
        false;
  }
  ...
  Future<bool> _connect(
    ...
    if (_isServerCommandOk(result[keyOk])) {
      ...
      if (!_isServerCommandOk(result[keyOk]) &&
        ...
        throw MongoDartError(
          result[keyErrmsg]?.toString() ?? 'Hello failed',
      ...
      if (_isServerCommandNotOk(documents.first[keyOk])) {
        var errorMessage =
            documents.first[keyErrmsg]?.toString() ??
    ...
    if (!authenticate) {
      _log.fine(
        () =>
            '$db: ${connection.serverConfig.hostUrl} '
            'topology connected',
      );
      return true;
    }
    ...
    var authFuture = () async {
      ...
      try {
        await db.authenticate(
          connection.serverConfig.userName,
        ...
        if (e is MongoDartError &&
          ...
          _log.warning(
            () =>
                'Atlas connection: SCRAM_SHA_256 not available, '
  ...
  Future<void> open(WriteConcern writeConcern) async {
    ...
    for (var hostPool in _connectionPool.values) {
      seedStartupTasks.add(
        _connectOpenSeed(
          hostPool.ensureOneConnection(),
          connectionErrors,
          primaryReady,
        ),
      );
    ...
    db._setLifecycleState(State.open);
  ...
  Future<void> _connectOpenSeed(
    Connection connection,
    List<Object> connectionErrors,
    Completer<Connection> primaryReady,
  ) async {
    try {
      ...
      if (!primaryReady.isCompleted) {
        _log.finer(
          () =>
              'Open seed startup failed for '
              '${connection.serverConfig.hostUrl}: $error',
        );
      } else {
        _log.warning(
          () =>
              'Background host startup failed for '
              '${connection.serverConfig.hostUrl}: $error',
        );
      }
    }
  ...
  Future<void> _refreshTopologyInternal() async {
    ...
    if (currentMaster != null &&
      ...
      try {
        var stillMaster = await _refreshConnectionRole(
          currentMaster,
      ...
      try {
        ...
        if (!promoted &&
          ...
          if (!hasAnyConnectedServer) {
            ...
            _log.finer(
              () =>
                  'Master recovery waiting for election; '
  ...
  Future<bool> _tryPromoteConnectedMaster({Connection? excluding}) async {
    for (var connection in [..._allConnections]) {
      ...
      try {
        var isMaster = await _refreshConnectionRole(
          connection,
          authenticateIfMaster: true,
        );
        if (isMaster) {
  ...
  Future<bool> _refreshConnectionRole(
    Connection connection, {
    bool authenticateIfMaster = false,
  }) async {
    if (_closed ||
        !identical(db._connectionManager, this) ||
        connection._closed ||
  ...
  void debugAddConnection(Connection connection) {
    var hostPool = _connectionPool.putIfAbsent(
      connection.serverConfig.hostUrl,
  ...
  bool debugHostContainsConnection(Connection connection) {
    return _connectionPool[connection.serverConfig.hostUrl]?._connections
            .contains(connection) ??
        false;
  }
}
...
class _HostConnectionPool {
  ...
  Connection? selectLeastBusyConnection({required bool requireAuthentication}) {
    ...
    var startIndex = totalConnections == 0
        ? 0
        : _selectionCursor % totalConnections;
  ...
  void handleConnectionFailure(Connection failedConnection) {
    ...
    var targetCount = min(
      _maxPoolSize,
      max(_minimumTrackedConnections, _connections.length + 1),
    );
