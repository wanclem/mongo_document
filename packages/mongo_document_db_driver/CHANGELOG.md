...
- Added bundled native libraries for the shipped targets: `macos-arm64`, `linux-x64`, and `windows-x64`.
- Introduced the async worker-isolate bridge so concurrent MongoDB operations no longer serialize behind blocking FFI calls on a single Dart isolate.
- Moved connection management, server selection, CRUD, aggregation, cursors, `getMore`, and change streams onto the Rust-backed execution path used by the package.
- Established `mongo_document_db_driver` as the low-level driver package name across the workspace.
...
- Synchronized package versioning across the repository (`mongo_document_db_driver`, `mongo_document_annotation`, and `mongo_document`).
...
- Reworked connection lifecycle for Atlas and long-running services:
  - Added resilient reconnect flow with bounded backoff.
  - Improved connection state transitions and socket teardown handling.
  - Added heartbeat/monitor behavior updates to reduce false closed-state failures.
- Removed `safeAtlas` workaround path from runtime connection flow.
- Internal refactors to improve stability under transient network interruptions.
...
- Fixed bug preventing pre 3.6 releases to work with mongo_document_db_driver
...
- The hidden class `_Connection` has been renamed `Connection`. If you have already your own Connection class may be that they conflict. In this case, you shoul hide the `Connection` class while importing MongoDart in your sources. Ex.```dart import 'package:mongo_document_db_driver/mongo_document_db_driver.dart' hide Connection;```
...
- Moved mongo_document_db_driver project to a new organization on [GitHub](https://github.com/wanclem/mongo_document).
...
- `authSource` connection string option added See: <https://github.com/vadimtsushko/mongo_document_db_driver/issues/72>
...
- Fix issue [71](https://github.com/vadimtsushko/mongo_document_db_driver/issues/71) - Logger allocates lot of strings
...
- Bugfix for [Issue 51](https://github.com/vadimtsushko/mongo_document_db_driver/issues/51) Can't reopen a closed database
- API docs redirected to dartdocs.org [Issue 48](https://github.com/vadimtsushko/mongo_document_db_driver/issues/48)

### 0.1.40

- Better error handling when a connection with the database is lost. Thanks to [luizmineo](https://github.com/luizmineo) [PR 50](https://github.com/vadimtsushko/mongo_document_db_driver/pull/50).
- `Future(List<String>) listCollections()` helper added to Db
- `Future(List<String>) listDatabases()` helper added to Db

### 0.1.39

- Better error handling. Bugfix for [issue 49](https://github.com/vadimtsushko/mongo_document_db_driver/issues/49)
...
getLastError(). See <https://github.com/vadimtsushko/mongo_document_db_driver/issues/41>
...
- Bugfix to [fields() issue](https://github.com/vadimtsushko/mongo_document_db_driver/issues/26). Fields clause in find(), findOne() methods did not work.
...
- mongo_dart_query published separately and added as dependency to mongo_document_db_driver. Unified SelectorBuilder will be used by mongo_document_db_driver and objectory.  

### 0.1.14

- Bson library published separately and added as dependency to mongo_document_db_driver  
...
- Bug fix on [Issue 18](https://github.com/vadimtsushko/mongo_document_db_driver/issues/18) about db.ensureIndex
...
- $err field set in MongoDB result object raises Error in mongo_document_db_driver
