...
void main() async {
  ...
  Future<bool> findServer(String uriString) async {
    var db = Db(_tlsUri(uriString, secure: true, tlsCAFile: caCertFile.path));
  ...
  Future<String?> getFcv(String uri) async {
    var db = Db(_tlsUri(uri, secure: true, tlsCAFile: caCertFile.path));
    try {
      await db.open();
  ...
  group('Server certificate', () {
    ...
    test('Must be run all together', () async {
      ...
      db = Db(_tlsUri(defaultUri, secure: true, tlsCAFile: caCertFile.path));
      await db.open();
      await db.close();
...
String _tlsUri(
  ...
  if (tlsCAFile != null) {
    queryParameters['tlsCAFile'] = tlsCAFile;
  }
