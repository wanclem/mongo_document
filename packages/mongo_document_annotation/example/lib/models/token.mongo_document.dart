...
const _tokenRefFields = <String>{};
const _tokenObjectIdFields = <String>{};
...
Map<String, dynamic> _tokenNormalizePersistedDocument(
  ...
  for (final key in _tokenObjectIdFields) {
...
Token _tokenDeserializeDocument(Map<String, dynamic> document) {
  ...
  return Token.fromJson(
    document.withRefs(
      refFields: _tokenRefFields,
      objectIdFields: _tokenObjectIdFields,
    ),
  );
}
...
ObjectId? _tokenCoerceDocumentId(dynamic rawId) {
...
class Tokens {
  ...
  static Future<List<Token?>> saveMany(List<Token> tokens, {Db? db}) async {
    ...
    final orderedIds = List<dynamic>.filled(tokens.length, null);
    final insertPositions = <int>[];
    for (int index = 0; index < tokens.length; index++) {
      final item = tokens[index];
      final json = _tokenNormalizePersistedDocument(item.toJson());
    ...
    for (final entry in toSave) {
      final docId = _tokenCoerceDocumentId(entry.$2['_id']);
      if (docId == null) continue;
      if (_tokenSnapshotFor(docId) == null) {
    ...
    if (uniqueMissingSnapshotIds.isNotEmpty) {
      final fetchedSnapshots =
          await coll
              ...
              .toList();
      rememberMongoDocumentSnapshots(_tokenCollection, fetchedSnapshots);
      for (final snapshot in fetchedSnapshots) {
        final snapshotId = _tokenCoerceDocumentId(snapshot['_id']);
        if (snapshotId == null) continue;
        fetchedSnapshotsById[snapshotId] = _tokenNormalizePersistedDocument(
          snapshot,
        );
      }
    }
    for (final entry in toSave) {
      ...
      final doc = entry.$2;
      final docId = _tokenCoerceDocumentId(doc['_id']);
      if (docId == null) continue;
      final updateDoc = Map<String, dynamic>.from(doc)..remove('_id');
      var snapshot = _tokenSnapshotFor(docId) ?? fetchedSnapshotsById[docId];
    ...
    return orderedIds
        ...
        .map((doc) => doc == null ? null : _tokenDeserializeDocument(doc))
        .toList();
  }
  ...
  static Future<Token?> findById(
    ...
    final token = await coll.modernFindOne(
      filter: {'_id': id},
      projection:
  ...
  static Future<Token?> updateOne(
    ...
    final cleanedSelector =
        predicate(QToken()).toSelectorBuilder().map.cleaned();
